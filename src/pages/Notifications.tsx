...
import { Search, RefreshCw, Mail, Smartphone, Loader2 } from 'lucide-react';
import { subscribeNotifications, updateNotificationStatus, Notification } from '../lib/db';
...
export default function Notifications() {
  ...
  const handleResend = async (id: string) => {
    try {
      await updateNotificationStatus(id, 'Pending');
      // Simulate network delay
      setTimeout(async () => {
        await updateNotificationStatus(id, 'Delivered');
      }, 2000);
    } catch (error) {
                ...
                {filteredNotifications.map((notif, index) => (
                  <tr key={notif.id} className={index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                      <button 
                        onClick={() => notif.id && handleResend(notif.id)}
